...
const { kebabCase, ensureDirExists } = require('./utilities')
...
const generateOperationFile = (config, operation, operation_name) => new Promise((resolve, reject) => {
  // CJD:  We only want to generate .controller, .module and .service files using this method
  if ((config.file_name.indexOf('.controller.') < 0) && (config.file_name.indexOf('.module.') < 0) && config.file_name.indexOf('.service.') < 0) {
    resolve();
    return;
  }
  ...
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    if (err) return reject(err);
    const subdir = path.relative(config.templates_dir, config.root)
    const target_dir = ensureDirExists(config.target_dir, path.join(subdir, pluralize.singular(operation_name)))
    const new_filename = config.file_name.replace('___', pluralize.singular(operation_name));
    const target_file = path.resolve(target_dir, new_filename);
    const template = Handlebars.compile(data.toString());

    const content = template({
      openbrace: '{',
      closebrace: '}' ,
      operation_name: pluralize.singular(operation_name).replace(/[}{]/g, ''),
...
const generateSchemaFile = (config, model, modelName) => new Promise((resolve, reject) => {
  // CJD:  We only want to generate .dto files using this method
  if ((config.file_name.indexOf('.dto.') < 0)) {
    resolve();
    return;
  }
  ...
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    if (err) return reject(err);
    const subdir = path.relative(config.templates_dir, config.root)
    const moduleName = modelName.split('_').pop()
    const target_dir = ensureDirExists(config.target_dir, path.join(subdir, kebabCase(moduleName), 'dto'))
    const new_filename = config.file_name.replace('___', kebabCase(modelName));
    const target_file = path.resolve(target_dir, new_filename);
    const template = Handlebars.compile(data.toString());
    ...
    const content = template({
      openbrace: '{',
      closebrace: '}' ,
      model_name: pluralize.singular(modelName).replace(/[}{]/g, ''),
      model: model,
      schemas: config.schemas,
      swagger: config.swagger
    });
 ...
 */
 const generateSchemaFiles = config => new Promise((resolve, reject) => {
  const files = {};
  _.each(config.schemas, (schema, modelName) => {
    if (modelName.match(/_Dto$/gi)) {
      const tmpModelName = modelName.replace(/_Dto$/gi, '');
      files[tmpModelName] = schema;
...
const generateOperationFiles = config => new Promise((resolve, reject) => {
  ...
  _.each(config.swagger.paths, (path, path_name) => {
    ...
    files[operation_name].push({
      path_name,
      path,
      subresource: (path_name.substring(operation_name.length+1) || '/')
    });
...
const generateDirectoryStructure = config => new Promise((resolve, reject) => {
  ...
  const schemas = _.get(config, 'swagger.swagger') === '2.0' ? _.get(config, 'swagger.definitions') : _.get(config, 'swagger.components.schemas')
